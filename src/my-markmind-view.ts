    ...
    if(!this.markMind){
      ...
      // 这里如果是打开新标签页，容器可能还没有渲染完成，无法执行命令，延迟一点时间
        setTimeout(() => {
          ...
          this.mindApp = createApp(SimpleMindMap, { leaf:this.leaf,viewId:this.viewId,mindFile:this.file,initMindData: this.markMindData,app:this.app,mode:'edit',initElementHeight:height,showMiniMap:true,contentEl:this.containerEl})
